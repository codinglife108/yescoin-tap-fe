...
const SuperTasks = () => {
    ...
    const story_meta =
        taskData?.story_meta?.length > 2
            ? JSON.parse(taskData?.story_meta)
            : null
    ...
    const fetchSettings = useCallback(async () => {
        try {
            ...
            let storyViews =
                response?.result?.settings?.story_supertask_view_ids
    ...
    const handleInviteFriends = async () => {
        ...
        try {
            const result = await fetchData('/friends/invite', {
                link: inviteLink,
                taskId: id,
            })
            // @ts-ignore
    ...
    useEffect(() => {
        const initInviteLink = async () => {
            const startAppParams = JSON.stringify({
                // @ts-ignore
                inviter: `r_${tg['initDataUnsafe']['user']['id']}`,
                task_id: id,
            })
            const linkRes = await fetchData('/user/invite-link', {
                link: btoa(startAppParams),
            })
            if (linkRes.result.isNew == false) {
                dispatch(
                    getDispatchObject(SET_TOAST, {
                        open: true,
                        message: 'You already have this invite link on channel',
                        type: 'link',
                    })
                )
            }
        ...
        return (
            <div className='flex gap-2'>
                ...
                {termConditionButtonLink ? (
                    <Button
                        onClick={() =>
                            window.open(termConditionButtonLink, '_blank')
                        }
                        className={'p-2 px-4 mb-3'}
                        style={{ fontSize: '1.05rem', background: '#262628' }}
                    >
                        📄 Terms & Conditions
                    </Button>
                ) : (
        ...
        <Panel>
            ...
            <div className='main-content'>
                <div
                    ...
                    onClick={() => {
                        if (taskData.stories?.length > 0) setShowStories(true)
                    }}
                >
                    ...
                    {taskData.total_reward > 0 && (
                        <p className='pr-1'>
                            {formatNumberWithSpaces(taskData.total_reward)}{' '}
                        </p>
                    )}
                    {/* <Icon16Chevron/> */}
                </div>
                ...
                <div className='banner-content mt-8'>
                    <div className='banner-card flex items-center gap-1'>
                        {taskData?.media_url?.endsWith('.mp4') ||
                            ...
                            <Img
                                radius={12}
                                src={taskData.logourl || taskData.media_url}
                                width={48}
                                height={48}
                                className={'mr-3 aspect-square object-cover '}
                                alt={taskData.title}
                                                ...
                                                >
                                                    ...
                                                    <div className='step-content'>
                                                        ...
                                                        {okxContext.isConfirming ? (
                                                            ...
                                                            (step.type ==
                                                                'walllet-connect' &&
                                                                okxContext.isWalletConnected) ||
                                                            (step.type ==
                                                                'claim_free_mnt' &&
                                                                okxContext.isClaimed) ? (
                                                            <p
                                                                className={
                                                                    'text-success'
                                                                }
                                                ...
                                                {isPencil &&
                                                    ...
                                                    Pencils.taskLearnMoreType && (
                                                        <span
                                                            ...
                                                            className={
                                                                'flex justify-center w-full cursor-pointer my-3'
                                                            }
                                                            onClick={() =>
                                                                window.open(
                                                                    Pencils.intruction_url,
                                                                    '_blank'
                                                                )
                                                            }
                                                        >
                                    ...
                                    >
                                        ...
                                        <div className='step-content'>
                                            ...
                                            {okxContext.isConfirming ? (
                                                ...
                                                (step.type == 'walllet-connect' &&
                                                    okxContext.isWalletConnected) ||
                                                (step.type == 'claim_free_mnt' &&
                                                    okxContext.isClaimed) ? (
                                                <p className={'text-success'}>
                                                    Completed!
                                                </p>
