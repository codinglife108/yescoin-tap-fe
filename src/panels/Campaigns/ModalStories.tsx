...
const ModalStories = ({images, onClose, taskdata, fetchSettings, fromWheel, hasViewedStories}: {
    images: string[],
    onClose: () => void,
    taskdata: any,
    fetchSettings: () => void,
    ...
    const handleTelegramClick = useCallback(() => {
        ...
        if (!isNotLastStep) {
            if (taskdata?.story_reward && taskdata?.story_reward > 0 && !hasViewedStories) {
                checkStoriesAward();
            } else {
                onClose();
            }
    ...
    const checkStoriesAward = useCallback(async () => {
        if (!taskdata) return;
        try {
            const response = await fetchData("/supertasks/checkStories", {id: taskdata.id});
            ...
            if (response.status !== "error") {
                setActiveModal(MODAL_INFO, {
                    ...
                    description: () => (
                        <p>{formatNumberWithSpaces(taskdata.story_reward)} YesCoin!</p>)
                });
                fetchSettings();
            }
            onClose();
    ...
    return (
        <div className="fixed z-50 inset-0">
            <div className="flex w-full gap-1">
                {images.length > 0 && (
                    <div className={"py-2 w-full absolute left-0 top-0 flex gap-2 px-1"}>
                        {images.map((e, index) => (
                            <div
                                key={`story${index}`}
                                className="bg-white flex-1 rounded-2xl"
                                style={{minHeight: "3px", opacity: index === selectedIndex ? 1 : 0.5}}
                            />
                        ))}
                    </div>
                )}
