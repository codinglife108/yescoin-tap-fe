...
const CampaignsComponent = () => {
    const [adsgramAdStatus, setAdsgramAdStatus] = useState(
        AdsgramAdStatus.SEARCHING
    )
    ...
    const fetchCampaigns = useCallback(async () => {
        try {
            ...
            const independentTasks = tasks.filter(
                (task: any) => !task.supertask_id && task.visible !== false
            )
            ...
            const superTasksWithSteps = superTasks
                .map((superTask: any) => ({
                    ...
                    steps: tasks.filter(
                        (task: any) => task.supertask_id === superTask.id
                    ),
                }))
            ...
            dispatch(
                getDispatchObject(SET_DAYLY_REWARD, responseDailyReward.result)
            )
            // setTimeout(() => {
    ...
    const check = async (task: any) => {
        const response = await fetchData('/tasks/check', { id: task['id'] })
        ...
        if (response.error) {
            dispatch(
                getDispatchObject(SET_TOAST, {
                    open: true,
                    message: t('taskNotSubscribedError'),
                    type: 'error',
                })
            )
            return
        ...
        dispatch(
            getDispatchObject(SET_TOAST, {
                open: true,
                message: `${formatNumberWithSpaces(task['award'])} Yescoin Received`,
                type: 'success',
            })
        )
        fetchCampaigns()
    ...
    const goToModal = (task: any) => {
        if (task['award'] === -1) {
            if (task['botaddress']) {
                // @ts-ignore
                tg.openTelegramLink(
                    `https://t.me/${task['botaddress'].replace('@', '')}`
                )
                return
            } else if (task['link']) {
                ...
                // @ts-ignore
                tg.openTelegramLink(
                    `https://t.me/${task['channelAddress'].replace('@', '')}`
                )
                return
            } else if (task['channeladdress']) {
                // @ts-ignore
    ...
    const categorizeCampaigns = (campaigns: any) => {
        return campaigns.reduce(
            (acc: any, campaign: any) => {
                ...
                const isCompleted =
                    campaign.award === -1 ||
                    (campaign.steps && campaign.total_reward === -1)
                const isDaily =
                    (campaign.daily && !isCompleted) ||
                    campaign.sectiontype == 'daily'
    ...
    const isWalletRewarded = userActivities?.hasOwnProperty(
        'mantleWalletConnectAt'
    )
    ...
    const getMantleTransReward = async () => {
        ...
        if (res) {
            const response = await fetchData('/tasks/getReward', {
                reward: 50000,
            })
            ...
            dispatch(
                getDispatchObject(SET_TOAST, {
                    open: true,
                    message: '50,000 Yescoin Received',
                    type: 'success',
                })
            )
        }
    ...
    const checkIfAdsgramAdIsAvailable = async () => {
        ...
        setAdsgramAdStatus(
            isAvailable
                ? AdsgramAdStatus.AVAILABLE
                : AdsgramAdStatus.NOT_AVAILABLE
        )
        return isAvailable
    }
    ...
    const playAdsgramAd = async () => {
        ...
        try {
            ...
            let storyViews =
                response?.result?.settings?.ads_watch?.watch_counter || 0
            if (storyViews >= 20) {
                dispatch(
                    getDispatchObject(SET_TOAST, {
                        open: true,
        ...
        if (!isAvailable) {
            window.alert(
                'Sorry, the Ad is not available anymore. Try again later.'
            )
            return
        }

        let continueAfterDisclaimer = window.confirm(
            'Ads are provided by third parties: this is not financial advice. Do your own research.'
        )
        ...
        try {
            ...
            if (!watchStatus.done || watchStatus.done !== true) {
                ...
                window.alert(
                    'You must watch the Ad till the end to collect your reward.'
                )
            } else {
                console.log('🟢 SUCCESS')
    ...
    const claimAdsgramAdReward = async () => {
        ...
        window.alert(
            `🥳 you were awarded ${formatNumberWithSpaces(ADSGRAM_AD_REWARD)} tokens!`
        )
    }
            ...
            <>
                ...
                >
                    ...
                    <div className='campaign-image-overlay py-2'>
                        <h2 className='campaign-title text-center text-[20px]'>
                            Connect Wallet
                        </h2>
                    </div>
            ...
            <Panel style={{ paddingBottom: 0 }}>
                ...
                <div className='campaigns-container'>
                        ...
                        {CATEGORIES.map((tab) => (
                            ...
                            >
                                ...
                                <span className='task-count'>
                                    {categorizedCampaigns[tab.id]?.length ||
                                        0 +
                                        (tab.id === 'completed' &&
                    ...
                    {loading ? (
                        ...
                        <>
                            {categorizedCampaigns[activeTab]
                            ...
                            <div
                                className={'tasks-panel mb-4'}
                                style={{
                                    //@ts-ignore
                                    display:
                                        activeTab === 'completed' &&
                                        categorizedCampaigns[activeTab].filter(
                                            (c: any) => !c.steps?.length
                                        ).length === 0 &&
                                        'none',
                                }}
                            >
                                {!loading &&
                                    ...
                                    activeTab === 'daily' &&
                                {
                                    /*((!dailyReward["completed"]) && activeTab === "daily")*/
                                } && (
                                        <>
                                            <DailyReward
