...
const Friends: FC = () => {
    ...
    const fetchFriends = async () => {
        ...
        const response2 = await fetchData('/friends/get-level2', {
            isNew: true,
        })
        if (response2.error) {
            return
    ...
    return (
        <Panel>
            ...
            <Collapse title={t('friendsList2')}>
                ...
                {!loading &&
                    friendsList2 !== null &&
                    friendsList2.friends &&
                    friendsList2.friends?.length > 0 && (
                        <>
                            {friendsList2?.friends?.map(
                                (friend: any, index: number) =>
                                    friend && (
                                        <Cell
                                            key={index}
