    ...
    return (
        <Panel>
            ...
            <div className={"justify-center flex"}>
                <img src={TeamLogo} width={"130px"} height={160} alt={"team-logo"}/>
            </div>
            <Placeholder
                title={t('teamJoinTitle')}
            />
            <p className={"text-center mt-4"}>{parseStringWithNewLine(t('teamJoinSubTitle'))}</p>
            <Spacing size={32}/>
                    ...
                    <Cell
                        ...
                        before={team.photo ? (
                            <div className={"teamAvatar--container"}>
                                <Img radius={100} src={getImageUrl(team.photo)}/>
                            </div>
                        ) : (
                            <div className={"teamAvatar--container"}>
                                <LetterAvatar radius={100} small width={36} height={36}>
                                    {team.name ? team.name.charAt(0).toUpperCase() : "Y"}
                                </LetterAvatar>
                            </div>
                        )}
                        after={<Icon16Chevron/>}
                        title={team.name}
                        onClick={() => navigate(ROUTE_TEAM, {state : {id: team.id}})}
                    >
                        <IconText
                            size="small"
