...
import {fetchData, getImageUrl, getTelegramImageUrlPlaceholder} from "../../utils/api";
...
const Team: FC = () => {
    ...
    const id = getTeamIdFromStateIfPresent(location) || getIdFromPathname(location);
    ...
    return (
        <Panel overflowToContent={true} style={{height: "100%"}}>
            ...
            <TelegramBackButton/>
            {loading && (
                <div className={"flex absolute inset-0 items-center justify-center w-full h-full min-h-full"}>
                    <CircularProgress/>
                </div>
            )}
            ...
            {!loading && team && (
                <>
                    <TeamHeader
                        image={team.image ? (
                            ...
                            <div className={"team-image-container large"}>
                                <LetterAvatar radius={60} width={60} height={60}>
                    ...
                    <Spacing size={8}/>
                    <CellContainer extraStyle={{maxHeight: "220px", overflowY: "auto"}}>
                        {team.topUsers && team.topUsers?.map((user, index) => (
                            <Cell
                                key={index}
                                before={<Img radius={100}
                                             src={getTelegramImageUrlPlaceholder(user.tgId.toString())}/>}
                                title={user.name}
                            >
