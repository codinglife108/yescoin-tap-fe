...
const PlayerRow = ({ name, coins }: { name: string; coins: number }) => (
    <div className='LeagueRankPage--playerRow'>
        ...
        <div className='LeagueRankPage--playerInfo'>
            ...
            <span className='LeagueRankPage--playerCoins'>
                {' '}
                <IconText
                    size='small'
    ...
    return (
        <Panel>
            ...
            <div className='LeagueRankPage--leagueIcon'>
                <ChevronLeft
                    className='LeagueRankPage--chevron'
                    onClick={() => changeLeague(-1)}
                />
                <div
                    className={`LeagueRankPage--iconWrapper ${isChangingLeague ? 'changing' : ''}`}
                >
                    <img
                        src={currentLeague.icon || league1Icon}
                        alt='League Trophy'
                    />
                </div>
                <ChevronRight
            ...
            <h1
                className={`LeagueRankPage--leagueName ${isChangingLeague ? 'changing' : ''} capitalize`}
            >
                {currentLeague.name.replace('league', '')}
            </h1>
            <p
                className={`LeagueRankPage--leagueDescription ${isChangingLeague ? 'changing' : ''}`}
            >
            ...
            <div className='LeagueRankPage--tabsPanel'>
                <div className='LeagueRankPage--tabs'>
                    <div
                        ...
                        style={{
                            transform: `translateX(${activeTab === 'Miners' ? '0%' : '0'})`,
                        }}
                    ></div>
                    <button
