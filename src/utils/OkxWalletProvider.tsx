...
// const OkxWalletContext = createContext<OkxWalletContextType | undefined>(undefined);
...
//   const isWalletConnected = userActivities?.hasOwnProperty("mantleWalletConnectAt")||true;
...
//     dispatch(getDispatchObject(SET_USER_ACTIVTY, response?.result?.settings));
//   }
...
//           const acc = universalUi.session?.namespaces?.eip155?.accounts[0];
//           setWalletAddress(acc?.split(':')?.[2]||null);
//         }
//       } catch (error) {
//         console.error('Failed to initialize OKXUniversalProvider:', error);
...
//         setWalletAddress(acc?.split(':')?.[2]||null);
//         if(acc) {
//           if(!userActivities?.hasOwnProperty("mantleWalletConnectAt")) {
//             await fetchData(
...
//           ]
//         }
//         var resOfTran = await universalUiState.request(data, "eip155:5000");
...
//   return (
//     <OkxWalletContext.Provider value={{ provider, connectWallet, disconnect, walletAddress, sendZeroTransaction, sendClaimTransaction, isWalletConnected, isClaimed, isConfirming }}>
//       {children}
