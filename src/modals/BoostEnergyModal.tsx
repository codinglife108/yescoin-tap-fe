...
import {DefaultStateType, getDispatchObject, REDUCE_GOLD, SET_DAILY_ENERGY} from "../store/reducer";
    ...
    return (
        ...
        >
            <ModalContent>
                {(onClose) => (
                    <>
                        <ModalHeader className="flex flex-col gap-1">More energy</ModalHeader>
                        <ModalBody>
                            <p className="text-16-medium">Select the desired amount of additional energy.<br/><br/>Available: {formatNumberWithSpaces(availableEnergy)} / 172,800</p>
                                ...
                                {ITEMS.filter((item) => item.energy <= availableEnergy).map((item, index) => (
                                    ...
                                    >
                                        <Img
                                            ...
                                            style={{ height: 24 }}
                                        />
                                        {formatNumberWithSpaces(item.energy)} — {formatNumberWithSpaces(item.cost)}
                                        <Img
