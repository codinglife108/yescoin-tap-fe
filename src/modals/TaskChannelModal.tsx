...
const TaskChannelModal: FC = () => {
    ...
    const goToChannel = () => {
        ...
        if (activeModalParams['channeladdress']) {
            // @ts-ignore
            tg.openTelegramLink(
                `https://t.me/${activeModalParams['channeladdress'].replace('@', '')}`
            )
            return
        }
        ...
        if (activeModalParams['botaddress']) {
            // @ts-ignore
            tg.openTelegramLink(
                `https://t.me/${activeModalParams['botaddress'].replace('@', '')}`
            )
            return
        }
            ...
            description: () => (
                <p>
                    {formatNumberWithSpaces(
                        activeModalParams.total_reward ||
                            activeModalParams.award
                    )}{' '}
                    YesCoin!
                </p>
            ),
    ...
    const checkTaskWithInput = async (onClose: () => void) => {
        ...
        dispatch(
            getDispatchObject(SET_TOAST, {
                open: true,
                message: `${formatNumberWithSpaces(activeModalParams.award)} Yescoin Received`,
                type: 'success',
            })
        )
                ...
                <ModalContent className={'bg-gray-800'}>
                    {(onClose) => (
                        <>
                            ...
                            <ModalBody>
                                {(activeModalParams['channeladdress'] ||
                                    activeModalParams['link'] ||
                                    activeModalParams['botaddress']) && (
                        ...
                        <>
                            ...
                            <ModalBody>
                                {(activeModalParams['channeladdress'] ||
                                    activeModalParams['link'] ||
                                    activeModalParams['botaddress']) && (
                                    <p className='text-16-medium text-center'>
                                        {'Tap here to claim the rewards'}
                                    </p>
                                )}
                            </ModalBody>
                            <ModalFooter>
                ...
                <ModalContent className={'bg-gray-800'}>
                    {(onClose) => (
                        <>
                            ...
                            <ModalBody>
                                {activeModalParams['additional_info']
                                    ?.description && (
                                    <p
                                        className='text-16-medium mb-0 mt-0'
                                        dangerouslySetInnerHTML={{
                                            __html: activeModalParams[
                                                'additional_info'
                                            ]
                                                ? activeModalParams[
                                                      'additional_info'
                                ...
                                <input
                                    className={
                                        'bg-white min-h-input-channel px-3 '
                                    }
                                    placeholder={'Input'}
                                    style={{
                                        borderRadius: 16,
