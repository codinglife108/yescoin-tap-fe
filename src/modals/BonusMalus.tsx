...
const BonusDescription = ({bonus}: { bonus: number }) => (
    <p className={"text-center text-md"}>
        ...
        <br/>
        <br/>
        Just a reminder: I start working 15 minutes after you go offline, and I can work for a maximum of 4 continuous
        hours.
    </p>
)
...
export default function BonusMalus({onClose, bonusMalusValue}: { onClose: () => void, bonusMalusValue: number }) {
    ...
    return (
        !screenPopup && <InformationModal
            floatingCenter
            buttonText={buttonText}
            learnMoreLink={isMalus ? "https://telegra.ph/How-does-the-Burn-works-08-09" : 'https://telegra.ph/How-does-the-tapbot-work-08-09'}
            isBoost={false}
            close={onClose}
            callback={() => {
            }}
            itemData={{
                icon: <img src={icon} width={140} height={140} alt={"modal-from-bottom"}/>,
                title,
