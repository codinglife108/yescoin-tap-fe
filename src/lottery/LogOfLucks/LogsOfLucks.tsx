        ...
        <div className='flex items-center justify-between gap-2'>
            ...
            <div className='flex flex-col gap-1 flex-1'>
                ...
                {!claimed && is_claimable ? (
                    <div
                        onClick={handleClaim}
                        className='LogItem--subtitle-claim'
                    >
                        CLAIM
                    </div>
                ) : (
    ...
    function fetchDbData() {
        fetchData('/wheel/getLogsOfLuck')
            .then((res) => {
                ...
                if (type === 'refferal') {
                    setData(
                        res.result?.logsOfLuck.filter(
                            (log: any) => log.is_refferal
                        )
                    )
                } else {
                    setData(
                        res.result?.logsOfLuck.filter(
                            (log: any) => log.prize === type
                        )
                    )
                }
                setLoading(false)
        ...
        <>
            <Panel>
                ...
                <span className='gap-3 flex flex-col'>
                    {data?.map(
                        (
                            log: Omit<LogItemProps, 'fetchDbData'>,
                            index: number
                        ) => (
                            <LogItem
                                key={index}
                        ...
                        itemData={{
                            ...
                            Description: () => (
                                <span className='text-center'>
                                    <p
                                        className='text-white text-center my-6 font-semibold px-4 text-xl'
                                        style={{ lineHeight: '1.05' }}
                                    >
                                        We are verifying that everything within
                                        the app has been completed correctly and
                                        that there have been no violations of
                                        the terms and conditions. Your rewards
                                        will be distributed within 90 days to
                                        the wallet linked to your account.
                                    </p>
                                </span>
                            ),
                            value: 0,
