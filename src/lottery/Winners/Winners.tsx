...
const WinnerItem = ({ won_time, username, prize, quantity }: WinnersProps) => {
  ...
  return (
    ...
    >
      ...
      >
        <strong className="capitalize" style={{ maxWidth: 80, minWidth: 80 }}>
          {title}
        </strong>
      </div>
      <div className={"flex flex-col gap-1 flex-1 opacity-50  "}>
        <span
          ...
          style={{ minWidth: 80, maxWidth: 80, textOverflow: "ellipsis" }}
        >
          @{username}
        </span>
      </div>
      ...
      <div className="flex flex-col items-end gap-1">
        <div
          ...
          style={{ minWidth: 80 }}
        >
          {timeAgo(won_time).giorni ? timeAgo(won_time).giorni + "D " : ""}
          {timeAgo(won_time).ore ? timeAgo(won_time).ore + "h " : ""}
          {timeAgo(won_time).minuti ? timeAgo(won_time).minuti + "m" : "0m"}
        </div>
        <div className="WinnerItem--date text-right opacity-50">{"ago"}</div>
      </div>
    </div>
  );
};
    ...
    <>
      <Panel>
        ...
        <span className="gap-2 flex flex-col">
          {data?.map(
            (log: Omit<WinnersProps, "fetchDbData">, index: number) => (
